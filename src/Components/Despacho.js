...
<Dialog open={this.state.open} onClose={handleClose.bind(this)} aria-labelledby="form-dialog-title">
                    ...
                    <DialogContent>
                        <DialogContentText>
                        Información importante, todos los campos son llenados de acuerdo a la disponibilidad de las maquinas 
          </DialogContentText>
          <form
        ...
        >
          ...
          <br/>
                         <InputLabel htmlFor="age-simple">Escoge Maquinas </InputLabel>
                         {this.state.Maqui.map((Maqui, idx) => (  
                                                   <div style={{ justifyContent: 'right' }}>

<br/>
<InputLabel htmlFor="age-simple">Escoge Máquina # {idx+1}</InputLabel> 
                          ...
                          <Select
                           ...
                           // value={Number(this.state.maquina)}
                            onChange={this.handleChangecombo_maquina(idx)}
                            input={<Input id="maquina" />}
                        >
...
function guardar(event) {
  ...
  const datoGuardar = {
    cotizacion_id: this.state.pedido_guardar.cotizacion_items[0].cotizacion_id,
             ...
             var newprod= this.state.Maqui.map((value, index) => {
              ...
              var fec_fin = value.fecha_final.getFullYear()+   "-" + ( value.fecha_final.getMonth() + 1) + "-" + value.fecha_final.getDate();
               ...
               //var newProductos = this.state.guardarMaq.concat({ maquina_id: value.target.value, fecha_inicio: value.fecha_maquina, fecha_fin: value.fecha_final, pedido_item_id:res6.data.id })
                this.setState({
                    guardarMaq: [...this.state.guardarMaq, { maquina_id: value.id.target.value, fecha_inicio:fec_ini, fecha_fin: fec_fin, pedido_item_id:res6.data.id }],
                })
              ...
              axios.post(process.env.REACT_APP_URL_LARAVEL + `/api/Maquina_asignacion/`, this.state.guardarMaq)
              .then(res7 => {
